...
void printNumArgs(int argc, int naturalNums){
  cout << "The number of command line arguments given: " << argc-1
       << endl; 
  cout << "The number of natural numbers given: " << naturalNums
       << endl << endl;
}
...
void print(int * naturalNums, int * primeFactorPointers[],
  ...
  cout << "Shared prime factors: " << endl;
  for (int i=0; i<arraySizes[size+1]; i++){
    cout << (int)sharedValues[i] << " ";
  }
  if (arraySizes[size+1] == 0){
    cout << "    No shared prime factors." << endl;
  }
  cout << endl;
}
