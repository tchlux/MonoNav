...
def random_response_from_list(lst):
    ...
    if type(lst[0]) is list:
        ...
        for i in range(length):
            repeat_number = lst[i][1]
            for i in range(repeat_number):
                temp_list.append(words_list[i][0])
...
# this set of functions is used to transform the current working dictionary to one with lists for each word and the number of occurences
#################################################################################
def value_in_list_bool(lst, value):
    if value in lst:
        return True
        pass
    for item in lst:
            ...
            if type(item) is list:
                for obj in item:
                    if obj == value:
                        return lst.index(item)
def fix_dictionary():
    dictionary_name = input('words_dictionary or big_dictionary? ')
    dictionary = return_a_file(dictionary_name)
    new_dictionary = {}
    for key in dictionary:
        new_dictionary[key] = []
        for item in dictionary[key]:
            temp_values_list = []
            if value_in_list_bool(new_dictionary[key], item):
                location = value_in_list_finder(new_dictionary[key], item)
                new_dictionary[key][location][1] += 1
            else:
                temp_values_list.append(item)
                temp_values_list.append(1)
                new_dictionary[key].append(temp_values_list)
    print('new_dictionary that has been calculated')
    save_a_dictionary(new_dictionary, 'big_dictionary')
fix_dictionary()
###################################################################################
