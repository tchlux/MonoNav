...
class Sudoku:
    ...
    def _check_values(self, low=VALUE_RANGE[0]):
        high = VALUE_RANGE[1]
        ...
        self._final_remarks()
#        self._multiple_solution_test()

    #Method:
    #Pre:   
    #Att's: 
    #Post:  
    def _multiple_solution_test(self):
        if self.stack.size == 0:
            self._final_remarks()
        else:
            self.solutions.append(self.stack)
            low = self._reset_and_retreat()
            self._check_values(low)
