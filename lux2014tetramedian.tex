...
%
\documentclass[12pt,journal,compsoc]{/usr/share/texmf-texlive/latex/IEEE/IEEEtran}
%
...
\title{Recursive Median Sample-based Blob Detection}
%
%
...
\author{Thomas~Lux% <-this % stops a space
\IEEEcompsocitemizethanks{\IEEEcompsocthanksitem T. Lux is a student of Computer Science and Mathematics, Roanoke College, Salem,
VA, 24153.\protect\\
E-mail: thlux@mail.roanoke.edu}% <-this % stops an unwanted space
}
%\thanks{Manuscript received July 19th, 2014}}


% The paper headers
\markboth{Roanoke College, July~2014}%
{Shell \MakeLowercase{\textit{et al.}}: Recursive Median Sample-based Blob Detection}
...
\begin{abstract}
In this paper a method for detecting convex patches of similar colors, otherwise referred to as blobs, is proposed.  The method is aimed at computational efficiency and hence relies on repeatedly sampling images to gather information.
\end{abstract}

% Note that keywords are not normally used for peerreview papers.
\begin{IEEEkeywords}
Computer Vision, Blob Detection, Sample Based, Relative
\end{IEEEkeywords}}
...
\IEEEPARstart{T}{his} algorithm is aimed at producing information about visual surroundings for the purpose of single-camera robotic navigation. The initial inspiration for this algorithm came from the abundance of image-processing based blob detection algorithms that required much of the image to be processed when only select portions were needed. Being sample based implies a certain level of inaccuracy in this algorithm. This approach is a means of gathering relative information about the current scene. Later in the paper a proper demonstration of where this type of algorithm would be useful is presented, along with image samples of its performance.
...
\section{Algorithm}
This blob detection algorithm relies on three primary operations to produce a set of blob centriods from a given image.
\begin{enumerate}
\item Sampling the image
\item Performing a recursive median search
\item Filtering and merging produced blobs
\end{enumerate}
\subsection{Sampling the Image}
...
\section{Conclusion}
These results demonstrate that a valid blob detection algorithm can be produced with mere image sampling.  The primary detriment of using a sampling based algorithm is deacreased accuracy, but this is intentionally sacrificed for speed of computation. The proposed blob detection algorithm has the potential to quickly identify similarly colored patches of images and their approximate center.

\subsection{Applications}
This algorithm can be applied for computationally efficient relative robotic navigation.  Although perfect obstacle avoidance cannot be guarenteed with this algorithm alone, in combination with other sensors this algorithm could provide a substantial foundation for navigating.  This algorithm would only require a single camera to operate.
...
\bibliography{lux2014tetramedian} %Cite the name of the bibliography file
...
\begin{IEEEbiography}{Thomas Lux}
Thomas Lux is a rising Junior Computer Science and Mathematics major at Roanoke College with a minor in Physics.  T. Lux first began his interests in the robotics and computer vision fields over the summer of 2013.  A project on various sensors used for robotic mapping provided an introduction to OpenCV and the field of computer vision that inspired this work.
\end{IEEEbiography}
