...
#define MAX_BLOBS_IN_OBJECT 15     // Max accepted computations per "object"

#define MIN_BLOBS_IN_OBJECT 5 // Minimum number of blobs in an object
                              // for conidence
#define MIN_LIFESPAN_TO_SHOW 21    // Min lifespan before valid
#define MAX_LIFESPAN_VALUE 255     // Max lifespan allowed
#define LIFESPAN_DECREMENT_RATE 5 // Rate at which it dies if not tracked
...
#define MAX_ACCELERATION Physics(2.0,2.0)
...
public:
  ...
  Physics velocity; // The blob's x and y velocity
  Physics acceleration; // The blob's x and y acceleration
  ...
  Blob(const Px &Color, const Pt &Lower, const Pt &Upper){
    ...
    lower = Lower;
    upper = Upper;
    velocity = acceleration = Physics(0.0,0.0);
  }
...
public:
  ...
  void updateObject(const Blob &blob){
    updateXYAverage(velocity, blob.velocity);
    updateColorAverage(color, blob.color);
    setMinXY(lower, blob.lower);
    setMaxXY(upper, blob.upper);    
  }
};
...
bool blobsSimilar(const Blob &blob1, const Blob &blob2){
  return ( channelsSimilar(blob1.color, blob2.color) &&
	   ...
	   xySimilar(blob1.lower, blob2.lower, g_position_difference_allowance) &&
	   xySimilar(blob1.upper, blob2.upper, g_position_difference_allowance));
}
...
void adjustMinMax(Blob &currentBlob, const Pt &newPt){
  setMinXY(currentBlob.lower, newPt);
  setMaxXY(currentBlob.upper, newPt);
}

// TODO:  This function updates the acceleration and velocity of the
//        first given blob with the second given blob
void updateBlobPhysics(Blob &toUpdate, const Pt previousCenter,
		       const Blob &newBlob){
  const Physics currentVelocity = (BLOB_CENTER(newBlob) - previousCenter);
  toUpdate.acceleration = (currentVelocity - toUpdate.velocity);
  setMinXY(toUpdate.acceleration, MAX_ACCELERATION);
  setMaxXY(toUpdate.acceleration, -MAX_ACCELERATION);
  toUpdate.velocity += toUpdate.acceleration;
}
...
public:
  ...
  bool trackBlob(Blob &blobToTrack) const{
    const Pt predictedCenter(BLOB_CENTER(blobToTrack.prediction()));
    Blob newBlob(img.at<Px>(predictedCenter.y, predictedCenter.x), 
		 predictedCenter, predictedCenter);
    if (channelsSimilar(blobToTrack.color, newBlob.color)){
      tetraMedianSearch(newBlob);
      updateXYAverage(blobToTrack.lower, newBlob.lower);
      updateXYAverage(blobToTrack.upper, newBlob.upper);
      updateColorAverage(blobToTrack.color, newBlob.color);      
      updateBlobPhysics(blobToTrack, predictedCenter, newBlob);
      return true;
    }
    return false;
  }
  ...
  void mergeObjects(){
	...
	if (blobsSimilar((*obj1), (*obj2))){ // If the two objects are similar
	  setMinXY(obj1->lower, obj2->lower); // Set the min of both objects
	  setMaxXY(obj1->upper, obj2->upper); // Set the max of both objects
  ...
  void detectBlobs(){
    ...
    //    std::cerr << "regularSamples: " << regularSamples << std::endl;
  }
  ...
  void drawObject(Img &outImg, const Object &obj){
    cv::Scalar velocityColor(obj.lifespan/2,obj.lifespan,0);
    if ((obj.lifespan > MIN_LIFESPAN_TO_SHOW) &&
	(obj.blobs.size() > MIN_BLOBS_IN_OBJECT)){
